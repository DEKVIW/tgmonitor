...
import {
  archivePanTransferPublishRecord,
  deletePanTransferPublishRecord,
...
const PublishSectionPanel = ({ refreshToken }: PublishSectionProps) => {
  ...
  const handleBatchAction = async (
    action: 'republish' | 'validate' | 'refresh' | 'archive' | 'delete'
  ) => {
      ...
      for (const record of selectedRecords) {
        try {
          if (action === 'republish') {
            ...
            await archivePanTransferPublishRecord(record.id)
          } else {
            await deletePanTransferPublishRecord(record.id)
          }
          successCount += 1
        } catch {
          failureCount += 1
    ...
    { title: '操作人', dataIndex: 'operator', key: 'operator', width: 120, render: (value?: string | null) => <Text>{value || '-'}</Text> },
    {
      ...
      render: (_, record) => (
        <div className="resource-ops-transfer-action-grid resource-ops-transfer-action-grid--wide">
          <Tooltip title="发布到前台"><Button size="small" type="text" loading={busyAction === `republish-${record.id}`} icon={<SendOutlined />} onClick={() => void runRecordAction(`republish-${record.id}`, () => republishPanTransferPublishRecord(record.id).then(() => undefined), '已重新发布到前台')} /></Tooltip>
          ...
          <Popconfirm title="确认归档这条资源吗？" onConfirm={() => void runRecordAction(`archive-${record.id}`, () => archivePanTransferPublishRecord(record.id).then(() => undefined), '已归档该资源')}><Tooltip title="归档"><Button size="small" type="text" icon={<InboxOutlined />} loading={busyAction === `archive-${record.id}`} /></Tooltip></Popconfirm>
          <Popconfirm title="确认删除这条运营发布记录吗？" onConfirm={() => void runRecordAction(`delete-${record.id}`, () => deletePanTransferPublishRecord(record.id).then(() => undefined), '已删除该资源')}><Tooltip title="删除"><Button size="small" type="text" danger icon={<DeleteOutlined />} loading={busyAction === `delete-${record.id}`} /></Tooltip></Popconfirm>
        </div>
      ),
    },
  ]
  ...
  return (
    <>
      <Card className="resource-ops-panel-card">
        ...
        <Alert type="info" showIcon style={{ marginBottom: 16 }} message="运营发布已经收敛为资源表" description="支持搜索、排序、批量发布到前台、批量校验三路链接、批量归档/删除，以及从链接标签右键直接查看目录。" />
        ...
        <div className="resource-ops-transfer-bulkbar">
          ...
          <Space wrap size={[8, 8]}>
            ...
            <Button disabled={selectedRecords.length <= 0} loading={batchActionLoading === 'archive'} onClick={() => void handleBatchAction('archive')}>批量归档</Button>

...
const ChannelManagerRefined = () => {
  ...
  const handleExportSamples = () => {
    ...
    anchor.download = `channel-samples-${sampleData.username}-${Date.now()}.json`
  ...
  const columns: TableProps<ChannelResponse>['columns'] = [
    {
      title: '频道用户名',
      dataIndex: 'username',
      key: 'username',
  ...
  const columnsWithParserProfile: TableProps<ChannelResponse>['columns'] = [
    ...columns.slice(0, 5),
    {
      title: '解析路线',
      key: 'parser_profile',
    ...
    <div className="channel-manager">
      ...
      {testResult && (
        <Alert
          ...
          description={
            testResult.success
              ? `已测试 ${testResult.channels_tested} 个频道，事件处理器已注册${testResult.message_received ? '，并收到消息' : ''}`
              : testResult.error
          }
      ...
      <Table
        columns={columnsWithParserProfile}
        dataSource={channels}
        rowKey="id"
        loading={loading}
        tableLayout="auto"
      ...
      >
        <div className="channel-sample-toolbar">
          <div className="channel-sample-toolbar-group">
            <Text strong>抓取条数</Text>
            <InputNumber min={1} max={20} value={sampleLimit} onChange={(value) => setSampleLimit(value || 8)} />
          </div>

          <div className="channel-sample-toolbar-group">
            <Text strong>只看含链接</Text>
            <Switch checked={sampleOnlyWithLinks} onChange={setSampleOnlyWithLinks} />
          </div>
          ...
          <Space wrap>
            <Button
              ...
              onClick={() => {
                if (sampleChannel) {
                  void loadSamples(sampleChannel, sampleLimit, sampleOnlyWithLinks)
                }
              }}
            >
              刷新样本
        ...
        {sampleChannel && (
          <Descriptions size="small" bordered column={2} style={{ marginBottom: 16 }}>
            ...
            <Descriptions.Item label="样本数量">
              {sampleData?.sample_count ?? 0} / {sampleLimit}
            </Descriptions.Item>
          </Descriptions>
        )}
        ...
        {sampleLoading ? (
          ...
          <>
            <Alert
              ...
              showIcon
              style={{ marginBottom: 16 }}
              message={`已返回 ${sampleData.sample_count} 条样本`}
              description="这里展示的是 Telegram 原始消息和当前解析结果，后面可以直接按这些固定样本继续收敛解析规则。"
            />
...
export default ChannelManagerRefined
